...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--name", type=str, default="test", help="exp/model name"
    )
    parser.add_argument(
        "--train_list",
        type=str,
    ...
    config = (
        json.load(open("configs/template/main_template_tiny.json", encoding="utf-8"))
        if args.tiny
        else json.load(open("configs/template/main_templatejson", encoding="utf-8"))
    )
    train = []
    val = []
    idx = 0
    ...
    with (
        ...
        config_diff = du.load_config("configs/template/diffusion_template.yaml")
        ...
        exp_dir = Path(f'exp/{args.name}')
        exp_dir.mkdir(exist_ok=True)
        config_path = exp_dir / 'config.json'
        logger.info(f"Writing to {config_path}")
        with open(config_path, "w", encoding="utf-8") as f:
            json.dump(config, f, indent=2)
        config_diff_path = exp_dir / 'config_diff.yaml'
        logger.info(f"Writing to {config_diff_path}")
        du.save_config(config_diff_path.as_posix(), config_diff)
