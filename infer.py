...
def main():
    ...
    # 一定要设置的部分
    parser.add_argument(
        "-m", "--model_path", type=str, default="exp/default/{lastest}", help="模型路径"
    )
    parser.add_argument(
        "-c",
        "--config_path",
        type=str,
        default="exp/default/config.json",
        help="配置文件路径",
    )
    parser.add_argument(
        "-t",
    ...
    parser.add_argument(
        ...
        type=str,
        default="exp/default/diffusion/config.yaml",
        help="扩散模型配置文件路径",
    )
    parser.add_argument(
        "-ks",
    ...
    for in_file, tran in zip(args.input, trans):
        ...
        for spk in spk_list:
            ...
            if use_spk_mix:
                spk = "spk_mix"
            out_file = in_file.with_name(f"{in_file.stem}_{key}_{spk}{cluster_name}_{isdiffusion}_{args.vocal_register_shift_key}vk.{wav_format}")
