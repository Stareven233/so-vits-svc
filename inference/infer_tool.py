...
class Svc:
    def __init__(
        ...
        config_path,
        device=None,
        cluster_model_path="exp/default/kmeans_10000.pt",
        nsf_hifigan_enhance=False,
        diffusion_model_path="exp/default/diffusion/model_0.pt",
        ...
        if self.shallow_diffusion or self.only_diffusion:
            if os.path.exists(diffusion_model_path) and os.path.exists(diffusion_model_path):
                self.diffusion_model, self.vocoder, self.args_diff = load_model_vocoder(diffusion_model_path, self.dev, config_path=diffusion_config_path)
        ...
        if not self.only_diffusion:
            ...
            self.hubert_model = utils.get_speech_encoder(
                self.args_diff.data.encoder, device=self.dev
            )
            self.volume_extractor = utils.Volume_Extractor(
                self.args_diff.data.block_size
            )
        ...
        with logger.Progress() as progress:
            for slice_tag, data in progress.track(audio_data):
                ...
                for k, dat in enumerate(datas):
                    ...
                    pad_len = int(audio_sr * pad_seconds)
                    dat = np.concatenate(
                        [np.zeros([pad_len]), dat, np.zeros([pad_len])]
                    )
                    raw_path = io.BytesIO()
