...
def slice_audio(
    ...
    from ..utils.slice_audio import slice_audio_file
...
def slice_audio_v2(
    ...
    from utils.slice_audio_v2 import slice_audio_file_v2
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for i in tqdm(as_completed(tasks), total=len(tasks), desc="Processing"):
            try:
                ...
                logger.error(f"Error when processing file `{e}`, skipping")
                skipped += 1
