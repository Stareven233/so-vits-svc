...
common_preprocess_form = {
    "skip_slice": {
        "type": "checkbox",
        "default": False,
        "label": I.common_preprocess.skip_slice_label,
        "info": I.common_preprocess.skip_slice_info,
    },
    "slicer_num_workers": {
        "type": "slider",
        "max": 16,
        ...
        "step": 1,
        "label": I.common_preprocess.slicer_num_workers_label,
        "info": I.common_preprocess.slicer_num_workers_info,
    },
    "encoder": {
        "type": "dropdown",
...
def infer_fn_proxy(fn):
    def infer_fn(params, progress):
        ...
        if (
            params["vocal_register_shift_key"] != 0
            and "PC" not in get_shared_vocoder_info().name
        ):
...
def preprocess_fn_proxy(fn):
    def preprocess_fn(params, progress):
        ...
        # 补回去一个文件夹
        workdir_location = system_config.pkg.workdir_location
        if workdir_location:
            # 如果设置了工作目录位置，创建软连接
            os.makedirs(workdir_location, exist_ok=True)
            os.symlink(workdir_location, "exp/workdir", target_is_directory=True)
        else:
            # 默认行为，创建到 /home/waas/exp
            os.makedirs("exp/workdir")

        logger.info(
            f"rename by index: {False if params['_model_name'] == 'So-VITS-SVC' else True}"
        )

        # For SoVITS, output directly to data/44k; for others, output to data/train/audio
        output_dir = (
            "data/44k" if params["_model_name"] == "So-VITS-SVC" else "data/train/audio"
        )

        auto_normalize_dataset(
            output_dir,
            False if params["_model_name"] == "So-VITS-SVC" else True,
            max_workers=params["slicer_num_workers"],
            skip_slice=params.get("skip_slice", False),
        )
