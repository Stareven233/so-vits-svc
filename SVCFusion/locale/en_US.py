...
class _Locale(Locale):
    ...
    class model_chooser(Locale.model_chooser):
        ...
        no_spk_option = (
            "Model not loaded, speakers will be displayed after loading the model"
        )
    ...
    class form(Locale.form):
        ...
        audio_output_3 = "Output Result/Mix"
    ...
    class main_ui(Locale.main_ui):
        ...
        unloaded_model_tip = "Model not loaded, please load a model first"
        ...
        mixing_tab = "Auto Mixing"

        detect_spk_tip = "Detected characters:"
        spk_not_found_tip = "No characters detected"

        vocal_key_shift_tab = "Vocal Key Shift"
        cuda_benchmark_tools_tab = "CUDA Benchmark"
        start_cuda_benchmark_btn_value = "Start CUDA Benchmark"
        move_folder_name_error = "Invalid model name"
    ...
    class train(Locale.train):
        ...
        pretrain_model_not_found_suggestion = '<div style="background: var(--block-background-fill); padding: 8px;">Current model has no available pretrained models, please install from <a href="https://huggingface.co/collections/SVCFusion/pretrain-683b2fcfa4edab5a2942ba57" target="_blank">here</a></div>'
        ...
        archiving_tip = "Archiving in progress, do not click repeatedly."
    ...
    class settings(Locale.settings):
        ...
        class pkg(Locale.settings.pkg):
            ...
            workdir_location_label = "Working Directory Location"
            workdir_location_info = "Set custom working directory location, leave blank to use default exp/workdir"
    ...
    class install_model(Locale.install_model):
        ...
        installing_tip = "Model installing, please wait"
        install_success = (
            "Installation complete! Please refresh the page to see the new model."
        )
        install_failed = "Installation failed"
        file_empty = "Please upload a model package or wait for the upload to complete."
        model_name_empty = "Please enter the model name."
        unsupported_format = "Unsupported model package format."
        unsupported_model = "This model does not support installation."
    ...
    class common_infer(Locale.common_infer):
        ...
        precision_info = "Inference Precision"
        precision_label = "Precision"

        vocoder_label = "Vocoder"

        unknown_vocoder_tip = "Unknown vocoder, please check options"
        vocoder_not_loaded_tip = "Vocoder not loaded, please check options"
        ...
        use_automix_label = "Auto Mix"
        use_automix_info = (
            "Whether to use auto mix (effective when removing instrumentals)"
        )

        automix_but_not_vocal_separation_tip = (
            "Auto mix requires enabling instrumental removal"
        )
        ...
        vocal_register_shift_label = "Vocal Register Shift"
        vocal_register_shift_info = "Vocal register shift, can use vocoder key shift to make the model sing a wider range"
        vocal_register_shift_no_support_tip = "Current vocoder does not support vocal register shift, this parameter will be ignored"
    ...
    class common_preprocess(Locale.common_preprocess):
        ...
        slicer_num_workers_label = "Slicer Workers"
        slicer_num_workers_info = (
            "Number of processes used by the slicer. Set to 1 if you run out of memory."
        )

        f0_label = "F0 Extractor"
        f0_info = "Model for pitch extraction/prediction"

        skip_slice_label = "Skip Slicing"
        skip_slice_info = "Skip audio slicing, use original audio files directly"
    ...
    class sovits(Locale.sovits):
        ...
        class preprocess(Locale.sovits.preprocess):
            ...
            num_workers_info = "Higher number is faster, only effective in old version"

            subprocess_num_workers_label = "Subprocess Workers"
            subprocess_num_workers_info = (
                "Higher number is faster, only effective in old version"
            )

            debug_label = "Enable Debug Mode"
            debug_info = "Output debugging information, not necessary for general use."

            use_old_preprocess_label = "Use Old Preprocessing"
            use_old_preprocess_info = (
                "Use old preprocessing method, only use when current method fails."
            )
    ...
    class vocoder_key_shift(Locale.vocoder_key_shift):
        keyshift_slider_label = "Key Shift"
        keyshift_slider_info = "Adjust the pitch of the audio by semitones."
        audio_input_label = "Input Audio"
        submit_btn_value = "Process"
        output_audio_label = "Output Audio"
        vocoder_dropdown_label = "Vocoder Selection"
        update_vocoder_info = "Select the vocoder to use"
        process_success_log = "Audio processing complete, saved to {output_path}"
        process_failed_tip = (
            "Audio processing failed, please check input or vocoder settings"
        )

    class auto_mixing(Locale.auto_mixing):
        title = "🎵 Smart Auto Mixing"
        description = "The smart auto mixing feature can automatically apply professional-level mixing processing to your vocals and accompaniment, making your work sound more professional."
        support_info = "Supports automatic optimization processing for various music genres and voice types."
        upload_warning = "⚠️ Note: Please upload clean vocal files and accompaniment files, avoid uploading pre-mixed audio."
        contributor_info = (
            "Thanks to @祡🍊 for providing smart auto mixing technology support!"
        )

        input_vocals_label = "Input Vocals"
        input_bgm_label = "Input Accompaniment"

        music_genre_label = "Music Genre"
        music_genre_info = "Select music genre for optimal mixing effects"
        music_genre_pop = "Pop Music"
        music_genre_rock = "Rock Music"
        music_genre_jazz = "Jazz Music"
        music_genre_electronic = "Electronic Music"
        music_genre_folk = "Folk Music"
        music_genre_classical = "Classical Music"

        voice_type_label = "Voice Type"
        voice_type_info = "Select voice type for targeted optimization"
        voice_type_male_low = "Male Voice (Low)"
        voice_type_male_high = "Male Voice (High)"
        voice_type_female = "Female Voice"
        voice_type_rap = "Rap"
        voice_type_vocal = "Harmony"

        advanced_options_title = "Advanced Options"
        professional_params_title = "Professional Parameter Adjustment"

        deesser_strength_label = "De-esser Strength"
        deesser_strength_info = "Reduce harsh 'S' sounds in vocals"
        deesser_strength_off = "Off"
        deesser_strength_light = "Light"
        deesser_strength_moderate = "Moderate"
        deesser_strength_heavy = "Heavy"

        compression_strength_label = "Compression Strength"
        compression_strength_info = "Control dynamic range for more even sound"
        compression_strength_light = "Light"
        compression_strength_moderate = "Moderate"
        compression_strength_heavy = "Heavy"

        eq_style_label = "EQ Style"
        eq_style_info = "Tone equalizer style adjustment"
        eq_style_neutral = "Neutral"
        eq_style_bright = "Bright"
        eq_style_warm = "Warm"
        eq_style_vintage = "Vintage"

        reverb_level_label = "Reverb Level"
        reverb_level_info = "Spatial sense and depth adjustment"
        reverb_level_dry = "Dry"
        reverb_level_subtle = "Subtle"
        reverb_level_light = "Light"
        reverb_level_moderate = "Moderate"
        reverb_level_heavy = "Heavy"
        reverb_level_extreme = "Extreme"

        echo_level_label = "Echo Level"
        echo_level_info = "Add echo effects"
        echo_level_off = "Off"
        echo_level_light = "Light"
        echo_level_moderate = "Moderate"
        echo_level_heavy = "Heavy"

        submit_btn_value = "Start Mixing"
        output_mixed_label = "Mixed Output"

        processing_start_log = "Starting smart auto mixing processing..."
        processing_complete_log = "Smart auto mixing processing complete"
