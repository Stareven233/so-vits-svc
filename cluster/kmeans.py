...
class KMeansGPU:
    ...
    def __init__(
        ...
        print(
            "free_mem/GB:", info.free / 1024 / 1024 / 1024, "minibatch:", self.minibatch
        )
    ...
    def fit_predict(self, X):
        ...
        with torch.no_grad():
            ...
            if self.minibatch * 10 // offset < batch_size:
                x = X[
                    torch.randint(0, batch_size, [int(self.minibatch * 10 / offset)])
                ].to(self.device)
            else:
                x = X.to(self.device)
            # print(x.device)
            self.centroids = _kpp(
                x, self.n_clusters, min(int(self.minibatch / 12 / offset), batch_size)
            )
            del x
            torch.cuda.empty_cache()
            # self.centroids = self.centroids.to(self.device)
            num_points_in_clusters = torch.ones(
                self.n_clusters, device=self.device, dtype=X.dtype
            )  # 全1
            ...
            for i in range(self.max_iter):
                iter_time = time()
                if (
                    self.minibatch < batch_size // 2
                ):  # 可用minibatch数太小，每次都得从内存倒腾到显存
                ...
                expanded_closest = closest[None].expand(
                    self.n_clusters, -1
                )  # [1000, 3098036]#int16#0~999
                mask = (
                    expanded_closest
                    == torch.arange(self.n_clusters, device=self.device)[:, None]
                ).to(X.dtype)  # ==后者是int64*1000
                ...
                num_points_in_clusters[matched_clusters] += (
                    counts  # IndexError: tensors used as indices must be long, byte or bool tensors
                )
